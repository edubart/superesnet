...
function optim.adamhd(opfunc, x, config, state)
   -- (0) get/update state
   local config = config or {}
   local state = state or config
   local lr = config.learningRate or 0.001
   local llr = config.learningLearningRate or 1e-8
   ...
   state.u:mul(0):addcdiv(math.sqrt(biasCorrection2)/biasCorrection1, state.m, state.denom)
